...
import { CAMPOS_CUSTO_PRODUTO, CUSTO_ITEM_VAZIO_PRODUTO } from "@/lib/estudo-custos";
import type { EstudoViabilidadeDetail } from "@/lib/types";

type ItemSnapshot = { descricao: string; unidade: string; quantidade: number };

/** Etapa 4 (ramo Produto): custos por item/lote — preenchimento manual, com atalho
 * "aplicar a todos" para os campos que costumam repetir (ex: % de margem). */
export function EtapaCustosProduto({
  estudo,
  onUpdated,
}: {
  ...
  const campos = CAMPOS_CUSTO_PRODUTO;
  const vazio = CUSTO_ITEM_VAZIO_PRODUTO as Record<string, number>;
